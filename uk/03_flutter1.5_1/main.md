...
Доброго дня! Мене звуть Євген Єфремов. Я веб-розробник, але цікавлюсь і іншими напрямками. Я хочу розповісти про фреймворк `Flutter` від `Google`, який призначений для розробки мобільних додатків у першу чергу. Стаття буде цікава як мобільним розробникам, які ще не спробували `Flutter`, так і розробникам іншого профіля, яких цікавить стан на сучасному мобільному ринку. Ну й ті, хто цікавляться мовою програмування `Dart`, теж щось знайдуть для себе.
...
А от у фреймоврка `Flutter` від `Google` інший підхід. Там існує канвас і весь рендерінг працює через нього. `Flutter` вміє рендерити як `Material Design` (Android), так і `Cupertino` (iOS). Для розробки використовується мова програмування `Dart`, яка компілюється у бінарний код, що добре впливає на швивдкість.
...
На даний час доступна версія **Flutter 1.5**. Тому її і будемо використовувати.

## Встановлюємо Flutter

Скажу одразу про моє оточення. Я буду використовувати операційну систему `macOS`, підключу `Android`-девайс та буду на ньому запускати додаток. Але можна все це робити і на інших операційних системах (`GNU/Linux`, `Windows`) та запускати
...
Звісно, цікаво ще спробувати якісь більш системні речі. Наприклад, та ж камера, датчики, push-сповіщення тощо. Якщо стаття буде комусь цікава, то можливо я ще щось спробую зробити на `Flutter`. Тому чекаю від вас побажань!
